...
class APReader:
    ...
    def connect(self):
        ...
        for group in [channelGroups[groupLen] for groupLen in channelGroups if groupLen > 0]:
            ...
            for channel in group:
                ...
                if re.match(r"([T|t]ime)|([Z|z]eit)|([T|t]emps)",channel.Name) is not None:
                    timeChannel = channel
    ...
    def read(self):
        ...
        with open(self.filepath, 'rb') as f:
            ...
            if self.fileID < 5010:
                print('Only catmanAP v4.x+ are officially supported. Use this with caution!')
    ...
    # def collectChannelsFiltered(self, filter: str) -> list[Channel]:
    #     chans = []


    #     for c in self.Channels:
    #         if re.match(filter, c.Name) is not None:
    #             chans.append(c)
    #     # for c in self.Channels:
    ...
    def transformWildcardFilter(filter: str) -> str:
        """Transforms a wildcard filter to regex.

        Args:
            filter (str): Simplified wildcard filter.

        Returns:
            str: Regex filter equivalent.
        """
        filter = filter.replace("*",".*")
        # filter = filter.replace("?",".+")  # deactivated, because it disables "?" support
        return filter

    def collectChannelsLike(self, filter: str) -> list[Channel]:
        """Collect channels that match the filter.

        Args:
            filter (str): Simplified wildcard filter.

        Returns:
            list[Channel]: Matched channels.
        """
        filter = APReader.transformWildcardFilter(filter)
        ...
        assert len(chans) > 0, f'No channels found for filter "{filter}".'
    ...
    def collectChannels(self, channel_names: list[str]) -> list[Channel]:
        ...
        for cname in channel_names:
            for c in self.Channels:
                if re.match(cname, c.Name) is not None:
                    chans.append(c)
